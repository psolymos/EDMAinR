...
Here the distribution charactersizes the $FDM$ and not the $FM$.
...
df$x <- 1:nrow(df)
p <- ggplot(data=df, aes(x=x, y=dist, ymin=lower, ymax=upper)) +
...
We can consider a landmark infliential with respect to the form difference
...
form differences (i.e. paitwise distances between the landmark and
...
p <- ggplot(data=df, aes(x=landmark, y=Tdrop, ymin=lower, ymax=upper, group=1)) +
  geom_ribbon(col="#0000ff40", fill="#0000ff40") +
  geom_hline(yintercept=T_test(fdm)$statistic, col=2) +
  geom_line() +
...
of specimens from the 2 objects in tha same diagram.
